...
- 2026-08-31锁定三组开头输入与质量双模式：产品基础信息必须已经包含目标类目和多稳定类型回答；日常Run默认compact，但仍保留21项门、机械全量检查与完整风险人口逐行复核；规则/结构/检查器变化和P1案例必须full。分类三种数据缺口准确传递后自动按允许缺口闭合，不等待运行途中人工确认。
...
- 下一次验证因本批质量合同和封包检查器发生变化，必须使用`full-regression`；回归锁定后，无规则/结构/检查器变化的日常Run才默认使用compact。仍需按各Skill剩余槽位补齐两个正常加一个边界案例，不能以人工确认或compact运行倒填P1。
