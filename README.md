...
flowchart TD
    ...
    SITE -->|"是"| LOGIN["固定八会话登录门：Amazon由用户逐任务登录；SIF/卖家精灵用本机已保存凭据；逐Task回执"]
    LOGIN --> MODE{"运行模式"}
...
1. **输入、版本与登录门**：Listing主任务把三组输入、目标Amazon类目、是否存在多个稳定产品类型细分、站点、品牌、当前SKU事实和原始直接竞品锁成当前Listing Run。`full_pipeline`读取单一主输入工作簿；`登录准备`只登记SIF/卖家精灵的非敏感账户别名、凭据引用和登录方式，绝不填写密码。默认`downstream_intake`接收已封存的01–05及manifest。站点摘要确认后立即下发固定八会话矩阵：六个Amazon拥有任务由用户逐任务手动登录，SIF/卖家精灵拥有任务使用浏览器已保存凭据或本机密码管理器；逐Task/host回执完成前不得查询、提问、导出或联想。缺失事实保持未知，不从标题、竞品或历史SKU猜测。
...
完整输入与站点锁定后，主任务立即进入固定八会话登录门。在**真正执行该模块的任务和浏览器会话**中逐项准备：商品审计、五维洞察、标签网页核验、痛点频率、痛点口语表达和关键词联想六个Amazon任务由用户手动登录；SIF和卖家精灵任务按输入表中的账户别名/凭据引用使用浏览器已保存凭据或本机密码管理器。每份回执绑定Task、host和派发ID，一个任务回执不能顶替另一任务。密码、验证码、Cookie和令牌不写进输入表、聊天、README、Skill、manifest公开包或Git。每次实际采集前仍按所属Skill复核登录，Git拉取不会迁移登录态。
