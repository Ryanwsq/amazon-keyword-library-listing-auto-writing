...
flowchart TD
    ...
    SITE -->|"是"| LOGIN["固定八会话登录门：Amazon由用户逐任务登录；SIF/卖家精灵可手动或用本机凭据；逐Task回执"]
    LOGIN --> MODE{"运行模式"}
...
1. **输入、版本与登录门**：Listing主任务把三组输入、目标Amazon类目、是否存在多个稳定产品类型细分、站点、品牌、当前SKU事实和原始直接竞品锁成当前Listing Run。`full_pipeline`读取单一主输入工作簿；`登录准备`只登记SIF/卖家精灵的非敏感账户别名、凭据引用和登录方式，绝不填写密码。默认`downstream_intake`接收已封存的01–05及manifest。站点摘要确认后立即下发固定八会话矩阵：六个Amazon拥有任务由用户逐任务手动登录，SIF/卖家精灵拥有任务按输入选择用户手动或本机已保存凭据；逐Task/host回执完成前不得查询、提问、导出或联想。缺失事实保持未知，不从标题、竞品或历史SKU猜测。
...
2026-09-11执行修复：SIF和卖家精灵均可在输入表选择`用户手动`，此时账户别名/凭据引用可空；已保存凭据模式仍需非敏感本机引用。八份拥有任务登录证明不减少，完成后立即扫描并派发已授权的就绪支线。已自动填满SIF/卖家精灵登录表单时允许一次登录点击恢复，但不读取/复制/补填密码，验证码或失败仍交用户。Amazon始终用户手动登录。
...
|---|---|---|
| [SIF官网](https://www.sif.com/) | 关键词：SIF竞品反查 | 有竞品流量词查询及所需字段访问权限的SIF账号；在SIF拥有任务会话登录。优先网页；用户明确无法网页登录，或已登录但完整官方导出失败/不完整时，按拥有合同保留失败证据及当前授权，核验同Task/host的同提供商MCP鉴权后备用。未登录本身不触发降级。 |
...
完整输入与站点锁定后，主任务立即进入固定八会话登录门。在**真正执行该模块的任务和浏览器会话**中逐项准备：商品审计、五维洞察、标签网页核验、痛点频率、痛点口语表达和关键词联想六个Amazon任务由用户手动登录；SIF和卖家精灵任务可用户手动登录，或按输入表账户别名/凭据引用使用本机已保存凭据。每份回执绑定Task、host和派发ID，一个任务回执不能顶替另一任务。密码、验证码、Cookie和令牌不写进输入表、聊天、README、Skill、manifest公开包或Git。每次实际采集前仍按所属Skill复核登录，Git拉取不会迁移登录态。

## 需要配置哪些MCP

| MCP | 本项目需要的只读数据能力 | 使用边界 |
|---|---|---|
| SellerSprite MCP | 精确关键词的历史月搜索量；关键词挖掘作为获准备用入口 | 趋势阶段是首选接口；挖掘阶段仍严格为卖家精灵网页完整官方导出 > 卖家精灵MCP，不因MCP已配置而跳过网页。准备有效密钥、所需接口权限和足够用量，并分别检查网页会员与MCP次数。 |
| SIF MCP | 获准竞品ASIN流量词及合同所需SIF证据字段 | 按SIF网页登录不可完成或完整导出失败的备用条件启用，保留授权/鉴权/原始响应及完整性门。竞争仍只用SIF的Top3点击份额和Top3转化份额。 |
