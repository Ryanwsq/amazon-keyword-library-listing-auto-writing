...
跨项目的站点映射、来源优先级和 01–10 / 关键词工作簿字段，可统一从[全流程输入输出与来源路由索引](docs/workflow-input-output-and-source-routing.md)进入；最终规则仍由其中链接到的拥有合同决定。

本轮验证的37项运行异常、7项写作/装配补记及已修复/待专项边界见[2026-09-14维护追踪](docs/validation-repairs-20260914.md)。新候选不代表本机任务已加载；历史Run和交付不热改。
...
```mermaid
flowchart TD
    U["用户输入：新品基础信息（最后一列选择市场）、新品基础配置、竞品对标ASIN"] --> L0["Listing主任务：锁定Listing Run、输入路径与SHA-256、站点路由、当前产品事实"]
    L0 --> SITE{"站点路由可闭合？"}
    SITE -->|"否"| SSTOP["停止：记录错站并通知用户介入"]
    SITE -->|"是"| LOGIN["统一引导八会话准备；新Run按各分支自己的会话与依赖放行，旧Run按原锁"]
    LOGIN --> MODE{"运行模式"}
    MODE -->|"full_pipeline：明确授权且本分支已登录"| A0["独立执行商品审计、五维洞察、痛点；标签等待五维"]
    MODE -->|"downstream_intake：默认"| A1["接收并核验已封存01–05与manifest"]

    LOGIN --> KH["构造AKW-LISTING-INTERFACE-v1关键词交接对象"]
...
1. **输入、版本与登录门**：Listing主任务把三组输入、目标Amazon类目、是否存在多个稳定产品类型细分、站点、品牌、当前SKU事实和原始直接竞品锁成当前Listing Run。`full_pipeline`读取单一主输入工作簿；`登录准备`只登记SIF/卖家精灵的非敏感账户别名、凭据引用和登录方式，绝不填写密码。默认`downstream_intake`接收已封存的01–05及manifest。站点摘要确认后立即下发固定八会话矩阵：六个Amazon拥有任务由用户逐任务手动登录，SIF拥有任务先验证MCP（官网仅备用），卖家精灵也先核验MCP，仅明确报错后提示用户登录官网；逐Task/host回执完成前不得查询、提问、导出或联想。缺失事实保持未知，不从标题、竞品或历史SKU猜测。
2. **购物助手与市场证据支线**：美国站使用Alexa for Shopping和英语，德国站使用amazon.de内的Rufus和德语。商品审计生成02、03；五维洞察生成01并供标签阶段生成05；痛点频率生成04。不同支线保留各自样本、来源和失败状态，技术失败不能写成“没有事实”或0。
...
flowchart TD
    ...
    C10 --> C11["只读列出浏览器、待登录网站、MCP权限/额度与表格运行时缺口；不访问业务网站"]
...
| 副任务 | `Amazon关键词词库｜Amazon联想采集｜main` | 输入后由用户在拥有会话登录，按Run锁定站点采集联想（US=10001；DE=80539） |
...
2026-09-14最新顺序：SIF竞品反查、卖家精灵扩词均MCP优先，仅明确MCP报错后由主任务提示用户在对应拥有任务登录官网并验证，才允许网站备用；零/缺字段/不完整不触发降级。MCP正常不等待网页登录，输入的网页手动/已保存凭据选择仅为备用偏好。八份拥有任务身份证明不减少，各分支自身就绪即派发；六个Amazon任务仍由用户手动登录，秘密不落盘。详见[来源路由](docs/workflow-input-output-and-source-routing.md)。

关键词默认先检查现行近30天完整词库复用资格，测试不等于全新采集；只有用户明确要求才禁用复用。卖家精灵扩词锁定提供商“最近30天”，不推断完整日历月。根因、修复落点和验证边界见[本次执行修复记录](docs/runtime-repairs-20260911.md)。

| 网站 / 能力 | 哪个任务使用 | 账号和登录要求 |
|---|---|---|
| [SIF官网](https://www.sif.com/) | 关键词：SIF竞品反查 | 有竞品流量词查询及所需字段访问权限的SIF账号；优先在SIF拥有任务核验MCP。只有MCP明确报错时，保留原因及未完成ASIN，再登录官网备用。MCP正常不要求先登录官网；错站、实际鉴权失败或权限不明仍停止并通知用户。 |
| [卖家精灵官网](https://www.sellersprite.com/) | 关键词：关键词挖掘 | MCP优先，只有明确MCP错误后才提示用户在拥有任务登录官网并验证后导出备用；保留同查询与每种子一个成功完整结果。 |
| 锁定站点的Amazon搜索联想 | 关键词：Amazon联想 | 输入与站点锁定后，由用户在实际拥有任务会话手动登录；Amazon-US=`amazon.com`/All/10001，Amazon-DE=`amazon.de`/Alle或页面实际等义项/80539。首选内置浏览器，无法稳定操作时按原合同使用普通Chrome备用。不得切到另一站点。 |
| Alexa for Shopping / Rufus | Listing：商品信息审计、五维洞察、痛点等获准上游任务 | Amazon-US使用Alexa和英语；Amazon-DE使用amazon.de内的Rufus和德语。须在对应任务实际会话登录并验证。德国Rufus无可靠New Chat且刷新可能保留上下文；面板已打开时不得再次点击开关。不可用时按所属Skill停止，不能换普通搜索、商品页或模型猜测。 |
...
完整输入与站点锁定后，主任务立即进入固定八会话登录门。在**真正执行该模块的任务和浏览器会话**中逐项准备：商品审计、五维洞察、标签网页核验、痛点频率、痛点口语表达和关键词联想六个Amazon任务由用户手动登录；SIF任务先确认MCP鉴权，官网仅备用；卖家精灵任务仍可用户手动登录，或按输入表账户别名/凭据引用使用本机已保存凭据。每份回执绑定Task、host和派发ID，一个任务回执不能顶替另一任务。密码、验证码、Cookie和令牌不写进输入表、聊天、README、Skill、manifest公开包或Git。每次实际采集前仍按所属Skill复核登录，Git拉取不会迁移登录态。

## 需要配置哪些MCP

| MCP | 本项目需要的只读数据能力 | 使用边界 |
|---|---|---|
| SellerSprite MCP | 精确关键词的历史月搜索量；关键词挖掘首选接口 | 趋势阶段是首选接口；挖掘阶段仍严格为卖家精灵MCP > 卖家精灵官网完整官方导出，MCP正常不前置打开网页，只有明确报错后才提示用户登录官网备用。准备有效密钥、所需接口权限和足够用量，并分别检查网页会员与MCP次数。 |
| SIF MCP | 获准竞品ASIN流量词及合同所需SIF证据字段 | 竞品反查的首选入口，不要求先试官网或逐Run再次批准MCP例外；当前任务鉴权、原始响应、查询锁与完整性门仍保留。MCP明确报错时才按拥有合同启用SIF官网备用。竞争仍只用SIF的Top3点击份额和Top3转化份额。 |
| Sorftime MCP | 精确完整关键词的历史月搜索量 | 只作为SellerSprite MCP趋势不可用时的备用，不参与关键词挖掘。一个Run的全部趋势词和月份只使用一个提供商；不跨源补月，中途切换须按原合同重跑全部趋势人口。 |
...
SIF应使用本人账号在官方MCP入口提供的地址、认证字段和密钥；此仓库不猜测其认证方式，也不把另一提供商的`secret-key`头套给SIF。若后台无法提供或新任务无法发现对应只读能力，报告“SIF MCP未配置/待验证”，不要宣称首选MCP已就绪；按SIF拥有合同登记不可用后准备官网备用，其他提供商顺序不变。
