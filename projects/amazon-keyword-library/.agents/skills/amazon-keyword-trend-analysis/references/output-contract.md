...
锁定分类工作簿名称/哈希、Sheet2总人口与通用词库资格人口、`通用词库资格=纳入`的F1–F3唯一Keyword_ID人口、`marketplace`及提供商查询站点参数、最新完整月、至少24完整月查询范围、来源优先级、选定提供商、趋势批次和规则版本。`marketplace`只允许`Amazon-US`或`Amazon-DE`；站点参数不一致时记录`marketplace_mismatch`、停止并通知用户介入，不静默切成US。`不纳入/待复核`、F4/F5、Sheet3、Sheet4及SKU事实不输入。

## Formal source

来源接口优先级固定为`SellerSprite MCP -> Sorftime MCP`。默认使用SellerSprite MCP精确关键词月搜索量；SellerSprite MCP不可用时可使用Sorftime MCP精确关键词月搜索量。SellerSprite和Sorftime网页端都不是趋势正式采集接口，Sorftime官网只用于账户、MCP配置与用量检查。选定正式来源后，全部关键词和月份只使用该提供商；主来源批次中途失败时，备用来源必须从头重跑全部人口，不能拼接已取得主来源月份。主来源恢复只在新Run重新成为首选。
