...
| `amazon-keyword-sellersprite-expansion` | `keyword.source.keyword-mining.query`、`keyword.source.sellersprite.paginate-and-verify` | MCP优先、仅报错后登录官网导出备用；有细分核心词时分别使用一级核心词和细分核心词，否则只用一级核心词；每种子一个成功完整导出、四字段机械去重和损失风险 | draft |
