...
本批执行控制只用于新锁：完整读取dispatch合同的六字段根绑定、来源query_lock和`checkpoint`。来源周期/过滤/人口必须在build前锁全，卖家精灵挖掘固定提供商最近30天，不推断完整日历月；新回合/压缩/恢复首个业务读写前重新核对当前dispatch和允许路径。SIF新Run固定MCP优先，以当前Run/Task/host真实鉴权和query/source_access锁直接执行，不要求额外例外批准或官网失败；仅MCP明确报错可记录原因后将未完成ASIN转已登录官网，错误站点/权限不明/鉴权失败不能自动降级。两个提供商只允许各自Skill规定的已填满登录表单一次点击恢复，空白/部分填充或任何挑战仍交用户；不读取、记录或补填凭据。
...
1. 读取项目知识索引、端到端流程、判断边界、本任务直接合同和运行优化合同，锁定run_type、Run、revision、站点、周期、全部规则版本及用户三项输入。目标Amazon类目和“是否存在多个稳定产品类型细分”必须包含在开头的产品基础信息中；若已提供不得再次询问，缺失时Run不得启动。用户不负责指定任何核心词。未明确为测试/回归/P1案例时run_type默认`production`。使用`scripts/runtime_contract.py`在本机Run目录生成并验证内容寻址运行合同，新构建默认且强制`source_policies.sif_competitor=mcp-first-error-only-20260914`（旧已锁协议只按其历史锁验证，不用于新build/reserve）；合同只锁定输入、规则拥有文件哈希、阶段键和执行器版本，不用规则摘要替代各Skill完整阅读。
...
5. 每次派发前先按被Git忽略的本机映射机械核对逻辑角色、固定任务标题、任务ID/host、执行cwd、Run_ID、revision和输出目录；任一不符时停止派发。Run开始时可并行预检SIF与卖家精灵MCP鉴权登录并只写无凭据preflight；SIF主入口状态为authenticated_mcp，官网备用为authenticated_web，source_access另绑定当前Run/Task/host、查询哈希及合格备用原因，不能互当。未登录时副任务只回传`awaiting_login`，由主任务提示用户登录并冻结对应分支，登录恢复后继续。SIF与卖家精灵主入口均无需先网页登录；仅明确MCP报错后提示用户登录官网备用，执行前完整读取`../../../docs/mcp-first-source-access.md`。身份门闭合后并行调度Amazon联想和卖家精灵扩词；来源副任务按各自入口优先级执行并记录入口类型、回退和完整性。等待SIF和两个分支全部正式回传；副任务只回传最小工作簿、manifest和紧凑状态。主任务不得因副任务不可用、未初始化、操作不便或已经看到证据而代跑来源模块，也不得用临时通用子代理替代固定长期副任务。
...
- SIF新Run以MCP真实鉴权先行；仅mcp_error时携带完整失败与原锁证据，将未完成ASIN转已登录官网。SIF官网备用与卖家精灵均由拥有副任务首次网页动作前验证登录，未登录只回传主任务`awaiting_login`；卖家精灵MCP正常不要求网页登录。Amazon联想普通Chrome只作为合同内备用入口；卖家精灵每个种子只保存一个成功完整结果（MCP分页或官网备用导出），仅MCP明确报错时才提示用户登录同提供商官网备用。入口切换不改变提供商、查询或字段，无法闭环时状态准确降级。
