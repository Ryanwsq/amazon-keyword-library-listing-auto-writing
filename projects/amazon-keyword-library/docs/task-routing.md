...
- Runtime contract hash and target stage key:
- Dependency stage status hashes:
...
并行任务各自记录输入版本、异常和完成状态；不得把一个分支完成推断成整组完成。Run开始时在本机忽略目录生成内容寻址run contract，并可并行执行SIF与卖家精灵MCP鉴权的无凭据preflight。每次派发都必须先通过上述固定任务身份门，同时核对目标stage key和依赖status；每个模块必须由通过身份门的拥有副任务执行并正式回传当前Run相对路径、输出/证据哈希、人口、状态、缺口和验证。只有stage key、哈希、人口和`completed/completed_with_gaps`状态完全一致才可复用。主任务不得因副任务不可用、未初始化、操作不便或已能看到证据而代跑，也不得用通用临时子代理替代；只能在用户了解影响后明确批准的一次性Run例外下接收已经产生的证据，并将例外留痕且排除P1。来源副任务默认只回传工作簿、Run相对路径、哈希、数量、完成状态和异常摘要，逐页/逐调用长响应正文只留本机Run目录，避免重复占用主任务上下文。

技术失败只向固定阶段图的后代传播：词频失败只阻断词频与装配，分类失败阻断竞争/趋势/装配但不撤销词频；来源或清洗失败仍阻断其全部后代。无依赖关系且已就绪的分支继续执行。最终装配仍必须等待所有适用分支，因此失败隔离不允许缺模块交付。

来源入口按合同锁定：Amazon联想首选Codex内置浏览器并允许普通Chrome备用；SIF新Run固定MCP优先，当前Run/Task/host的authenticated_mcp和查询锁闭合即可启动，无先官网失败或额外逐Run批准。MCP明确报错时记录mcp_error，仅未完成ASIN转已登录官网；不明权限/错站/鉴权失败仍阻断。SIF官网备用和卖家精灵在首次网页动作前验证登录，未登录只回传主任务`awaiting_login`由其引导，不能借登录偏好改变来源优先级；卖家精灵则仅MCP明确报错时才提示用户登录同提供商官网备用。备用入口不得改变站点、查询、周期、过滤、字段或完整性门；只有稳定来源事件标识并能证明无遗漏/无重复时才可续采，否则重启受影响完整查询。趋势任务按`SellerSprite MCP -> Sorftime MCP`路由，网页端不作为正式趋势采集入口；同一Run全部趋势人口只使用一个提供商。
