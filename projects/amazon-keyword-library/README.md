...
- 远端仓库：`Ryanwsq/amazon-keyword-library-listing-auto-writing`；`main`是唯一长期基线。可见性和获准共享范围在发布前核实，不以静态说明代替授权。
...
- Skill文件依赖：`contracts/skill-dependencies.json`；由`scripts/validate_skill_dependencies.py`只读校验并接入原P0，不替代业务判断、完整阅读或Run锁。
