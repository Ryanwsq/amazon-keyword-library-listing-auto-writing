...
| `keyword_entry` | 新执行控制20260911必填。默认`{policy: recent-library-reuse-first}`，由关键词主任务先查现行近30天完整词库复用资格；测试Run不等于必须新采集。仅用户明确要求全新采集时为`{policy: fresh-by-user-request, source: {path, sha256, locator}}`，主任务必须核对原指令，不得凭“测试/开始/已登录”编造禁止复用。 |
...
| 键 | 要求 |
|---|---|
| `marketplace` / `brand` | marketplace必填且只允许`Amazon-US`或`Amazon-DE`；品牌未提供时保留null并说明，不借历史品牌补齐。 |
| `marketplace_route` | 由marketplace派生的`domain/postal_code/shopping_assistant/prompt_language`，双方逐字段核对。 |
...
`keyword_entry`仅把已有复用入口变成显式派发准备项，不替代关键词拥有合同的同站点/同类目/条件式同细分/原始最终输出0–30天/完整性判断。Listing不自判合格、不读取未获准历史路径；关键词主任务回传资格及原来源血缘。新Run的本机dispatch board以`keyword_request={path,sha256}`绑定完整请求；`dispatch_plan.py`核对当前Run、输入、站点和入口policy，缺失只阻断关键词派发并提示主任务补准备，不冻结独立Alexa分支。旧Run不补写新控制版本。
