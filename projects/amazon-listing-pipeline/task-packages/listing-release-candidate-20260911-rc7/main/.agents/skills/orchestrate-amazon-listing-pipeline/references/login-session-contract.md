...
3. SIF和卖家精灵任务可按`登录准备`使用浏览器已保存凭据或本机密码管理器，也可由用户选择手动登录，此时不要求填写凭据引用；引用只能是本机项目/条目名称，不能是密码。Amazon必须由用户在每个拥有Amazon动作的任务中手动登录。
4. 每个任务验证自己的页面、身份和站点后生成一份本Run本地证据并独立回执。主任务不能用自己的Cookie、截图或“其他任务已登录”代替。
5. 八份回执全部就绪后才完成`login_gate`；在此之前不得查询、提问、导出或采集联想。完成后立即全图扫描并实际派发所有可执行分支，不再等待用户重复说“开始”；已授权范围之外的动作仍不自动执行。
...
| `keyword:sif-collector:sif` | 关键词SIF竞品反查 | SIF | `keywords` | 用户手动、已保存凭据或本机密码管理器 |
...
SIF备用按关键词拥有合同：用户明确无法网页登录，或已登录但完整官方导出无法落盘/不完整，保留失败证据后可切同提供商MCP；仍须当前Run授权引用和本Task/host鉴权，不因未登录自动切换。已完整成功的ASIN不重复查询，来源人口/周期/字段不变。卖家精灵/SIF登录失效时，只有本机浏览器已自动填满账户和密码才允许点一次登录并复核原锁；不得读取、复制、显示、补填或记录凭据。空白/部分填充、验证码/MFA、失败、错账号/站点或再次跳转仍交用户处理。
