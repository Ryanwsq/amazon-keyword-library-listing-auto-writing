...
class CopyGateTests(unittest.TestCase):
    ...
    def test_init_defaults_to_version2_without_mutating_input(self):
        ...
        self.assertEqual("inline",created["quality_execution"]["final_checks"])

    def inline_receipt(self):
        self.confirm()
        self.manifest["quality_execution"] = {"final_checks":"inline","independent_qa":"not_requested","p1":False}
        output=self.root/"10-final.xlsx"
        output.write_bytes(b"synthetic final output")
        locks={str(output):pipeline.sha256_file(output)}
        self.manifest["stages"]["listing_generation"].update(status="completed",outputs=list(locks),output_sha256=locks)
        evidence=self.root/"owner-checks.json"
        evidence.write_text(json.dumps({"synthetic":True}))
        receipt={"run_id":self.manifest["run_id"],"kind":"assembly_self_check",
                 "checker_sha256":pipeline.sha256_file(Path(pipeline.__file__)),
                 "rule_locks":[{"path":str(Path(pipeline.__file__).resolve().parents[1]/"references"/"quality-gates.md"),
                                "sha256":pipeline.sha256_file(Path(pipeline.__file__).resolve().parents[1]/"references"/"quality-gates.md")}],
                 "input_sha256":self.manifest["input"]["sha256"],
                 "copy_sha256":self.manifest["copy_checkpoint"]["copy_sha256"],
                 "output_sha256":locks,"independent_qa":"not_executed","p1":False,"gaps":[],
                 "checks":{key:{"status":"pass","evidence":[{"path":str(evidence),"sha256":pipeline.sha256_file(evidence)}]} for key in pipeline.INLINE_CHECK_IDS}}
        path=self.root/"inline-receipt.json"
        path.write_text(json.dumps(receipt))
        self.save()
        return path,receipt,output

    def test_inline_checks_accept_once_without_independent_qa(self):
        path,_,_=self.inline_receipt()
        args=argparse.Namespace(manifest=str(self.path),receipt=str(path))
        with contextlib.redirect_stdout(io.StringIO()):
            pipeline.cmd_finish_inline_checks(args)
            before=self.path.read_bytes()
            pipeline.cmd_finish_inline_checks(args)
        self.assertEqual(before,self.path.read_bytes())
        result=pipeline.load_manifest(self.path)
        self.assertEqual("COMPLETED",result["overall_status"])
        self.assertFalse(result["quality_execution"]["p1"])

    def test_inline_checks_reject_missing_coverage_or_drift(self):
        path,receipt,output=self.inline_receipt()
        receipt["checks"].pop("fact_evidence")
        path.write_text(json.dumps(receipt))
        args=argparse.Namespace(manifest=str(self.path),receipt=str(path))
    ...
    def test_inline_qa_cannot_be_forced_without_receipt(self):
        self.inline_receipt()
        with self.assertRaisesRegex(ValueError,"finish-inline-checks"):
            pipeline.cmd_set_stage(argparse.Namespace(manifest=str(self.path),stage="final_qa",status="completed",force=True,message=None,output=[]))
    ...
    def test_complete_approval_locks_exact_copy_and_sources(self):
        ...
        self.assertEqual({str(self.draft.resolve()): pipeline.sha256_file(self.draft)},
                         self.manifest["stages"]["copy_checkpoint"]["output_sha256"])

    def test_confirmed_copy_is_immediately_dispatchable_without_manual_hash_patch(self):
        import dispatch_plan
        self.confirm()
        for record in self.manifest["stages"].values():
            if record["status"] == "pending":
                record["status"] = "completed"
        self.manifest["stages"]["listing_generation"]["status"] = "pending"
        self.manifest["stages"]["final_qa"]["status"] = "pending"
        board = {"schema": "listing-dispatch-board/v1", "run_id": self.manifest["run_id"], "jobs": {}}
        result = dispatch_plan.plan(self.manifest, board, "downstream_intake")
        actions = {r["node"]: r["action"] for r in result["nodes"]}
        self.assertEqual("completed", actions["copy_checkpoint"])
        self.assertEqual("dispatch", actions["listing_generation"])

    def test_new_control_requires_text_bound_bullet_review_without_length_rule(self):
        self.manifest["execution_controls_version"] = "20260911"
        with self.assertRaisesRegex(ValueError, "Bullet review"):
            pipeline.verify_copy_payload(self.manifest, self.draft)
        self.payload["bullet_review"] = [dict(
            bullet_number=i + 1, text_sha256=pipeline.hashlib.sha256(text.encode()).hexdigest(),
            purchase_reason="synthetic reason", mechanism_or_fact="synthetic fact", user_action_or_context="synthetic use",
            benefit="synthetic benefit", review_note="synthetic review", status="可优化",
            source_refs=[dict(path=str(self.facts), sha256=pipeline.sha256_file(self.facts), locator="新品基础配置!D2")])
            for i, text in enumerate(self.payload["bullet_points"])]
        self.draft.write_text(json.dumps(self.payload))
        pipeline.verify_copy_payload(self.manifest, self.draft)
        self.payload["bullet_points"][0] = "Changed"
        self.draft.write_text(json.dumps(self.payload))
