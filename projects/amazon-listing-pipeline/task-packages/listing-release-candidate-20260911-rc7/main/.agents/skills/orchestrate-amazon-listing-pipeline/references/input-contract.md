...
固定三行服务：`SIF`、`卖家精灵`、`Amazon`。SIF与卖家精灵可选择`用户手动`（账户别名/凭据引用可空），或选择`浏览器已保存凭据`、`本机密码管理器`（必须填写账户别名及本机非敏感凭据引用，例如浏览器配置名或条目名）。Amazon固定为`用户手动`，账户别名和凭据引用不填写。选择手动不等于已登录，仍须本Run的实际拥有任务独立回执。
...
完整输入与站点锁定后，`full_pipeline`按[任务会话登录合同](login-session-contract.md)先进入固定八会话登录门。主任务立即引导用户准备全部会话：SIF/卖家精灵任务按本Sheet选择用户手动或使用本机已保存凭据，六个Amazon拥有任务由用户逐任务手动登录。未全部回执前不得发起查询、提问、导出或联想；主任务或其他会话的Cookie不能代替拥有任务。
