...
REQUIRED_SHEETS = {
    "新品基础信息": ["ASIN", "产品类型", "产品人群画像", "使用场景", "用途", "核心卖点"],
...
def parse_config(rows: list[list[Any]]) -> dict[str, str]:
    result: dict[str, str] = {}
    for row in rows[1:]:
...
def main() -> int:
    ...
    try:
        ...
        config_name = find_sheet(list(reader.sheets), "运行配置")
        if config_name:
            rows, cell_errors, merges = reader.rows(config_name)
            errors.extend(f"公式/单元格错误：{item}" for item in cell_errors)
            if merges:
                warnings.append(f"运行配置: 存在合并单元格 {merges}")
            if validate_headers("运行配置", rows, CONFIG_HEADERS, errors, warnings):
                result["config"] = parse_config(rows)
        else:
            warnings.append("缺少运行配置 Sheet，使用默认值")

        try:
            formal_min = int(result["config"].get("formal_sample_min", "10") or "10")
            if formal_min < 1:
                raise ValueError
        except ValueError:
            errors.append("运行配置.formal_sample_min 必须为正整数")
            formal_min = 10
