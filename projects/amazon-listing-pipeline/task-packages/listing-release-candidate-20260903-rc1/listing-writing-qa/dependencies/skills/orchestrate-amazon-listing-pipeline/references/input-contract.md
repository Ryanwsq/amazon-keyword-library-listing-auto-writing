...
| 配置项 | 默认值 | 作用 |
|---|---|---|
| marketplace | Amazon US | 全流程站点 |
| alexa_language | natural English | Alexa 提问语言 |
| formal_sample_min | 10 | 正式市场与痛点样本最低数 |
| keyword_skill | 空白 | 指定关键词库生成 Skill 名称 |
| keyword_output_pattern | *SKU可用关键词库*.xlsx | 外部关键词输出发现规则 |
| run_painpoint_phrasing | selected_only | 仅对确认主张涉及的痛点运行 |
| checkpoint_candidate_count | 3 | 人工确认候选数量，最大 3 |
| search_terms_byte_limit | 249 | Amazon US 默认 UTF-8 字节上限 |

未知配置项保留但忽略，不得猜测其业务含义。
...
新建schema2.0 Run时，`pipeline_state.py init` 必须传入 `--product-asin <已核对的本品ASIN>`，登记为 `input.product_asin` 并与该次锁定输入哈希一起保留。主线程先把该值与输入或封存清单核对；状态脚本只检查格式和后续身份一致性，不替代工作簿事实预检。不修改已有schema1.0 Run。
