...
2026-09-04候选rc2经`AKW-DE-VALIDATION-FIXES-20260904-01`授权，补充输入站点锁、Amazon-US/Amazon-DE路由、输入完成后的Amazon/SIF/卖家精灵统一人工登录门、德国站Rufus固定德语问题及不可独立上下文例外、站点专属硬规则和告警/阻断语义。旧候选与历史Run不可变；本批只进行结构和机械回归，`business_run=not_executed`、`P1=not_executed`，下一次规则/Skill能力验证必须使用`test-validation + full-regression`。
