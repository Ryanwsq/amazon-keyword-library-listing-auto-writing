...
Use the authenticated shopping-assistant browser for the Run marketplace. After input and marketplace are locked, open the matching Amazon site in this task and wait for the user to log in manually. Do not ask questions until this session returns `authenticated`. A different marketplace is `marketplace_mismatch` and requires user intervention.
