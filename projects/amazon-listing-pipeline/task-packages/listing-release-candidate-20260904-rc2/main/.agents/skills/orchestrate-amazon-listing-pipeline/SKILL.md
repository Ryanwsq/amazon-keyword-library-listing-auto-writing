...
5. `full_pipeline`锁定输入和站点后，先在实际拥有任务中打开本Run要使用的Amazon站点、SIF和卖家精灵官网，等待用户手动完成全部网页登录。每个会话必须独立回执`authenticated`，再用`scripts/pipeline_state.py confirm-login`同时登记三份无凭据回执；未完成`login_gate`前状态机拒绝商品审计、五维洞察、标签网页核验、痛点、关键词和痛点口语提问进入`running/completed`。凭据/Cookie不进入Run或Git。`downstream_intake`不重跑已封存网页采集，但新的关键词采集仍受此门约束。
