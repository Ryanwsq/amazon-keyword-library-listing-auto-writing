...
- Run锁定站点的Amazon Listing与图片核验属于执行动作，不是第三个用户输入；US使用amazon.com/10001，DE使用amazon.de/80539。输入与站点锁定后先等待用户在本任务Amazon会话手动登录，取得`authenticated`回执后才核验；进入其他站点时停止并通知用户介入。
