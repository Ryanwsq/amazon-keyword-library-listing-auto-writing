...
LOGIN_METHODS = {"浏览器已保存凭据", "本机密码管理器", "用户手动"}
...
class XlsxReader:
    ...
    def rows(self, sheet_name: str, *, preserve_row_numbers: bool = False) -> tuple[list[list[Any]], list[str], list[str]]:
...
def validate_login_profile(service, alias, credential_ref, login_method, row_number, errors):
    """Manual login needs no stored secret reference; it still needs owner-session proof."""
    if service in {"SIF", "卖家精灵"} and login_method == "MCP鉴权":
        return  # Only an input choice; the owner must still prove current-task authentication.
    if service == "Amazon":
        if login_method != "用户手动":
            errors.append("登录准备: Amazon登录方式必须为用户手动")
        return
    if login_method not in LOGIN_METHODS:
        errors.append(f"登录准备!D{row_number}: {service}登录方式只允许 {sorted(LOGIN_METHODS)}")
    if login_method != "用户手动":
        if not alias:
            ...
            if validate_headers("登录准备", rows, LOGIN_HEADERS, errors, warnings):
                ...
                for row_number, row in enumerate(rows[1:], start=2):
                    ...
                    if service in profiles:
                        ...
                        continue
                    validate_login_profile(service, alias, credential_ref, login_method, row_number, errors)
                    profiles[service] = {
                        "account_alias": alias,
