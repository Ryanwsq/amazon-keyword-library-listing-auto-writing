...
SIF_WEB_FALLBACK_REASONS = {"mcp_unavailable", "mcp_incomplete"}
...
# Gate 7 coverage identifiers, not a second set of business thresholds.
INLINE_CHECK_IDS = (
    "approved_copy", "title_highlights", "bullet_policy", "backend_limits_disclosed",
    "fact_evidence", "keyword_source", "coverage_and_st_bytes", "priority_coverage",
    "prohibited_content", "fourteen_sheets_and_source_fidelity", "priority_traceability",
    "soft_writing_review",
)
MARKETPLACE_ROUTES = {
    "Amazon-US": {
...
def build_login_sessions(marketplace: str) -> dict[str, Any]:
    ...
    for source in LOGIN_SESSION_REQUIREMENTS:
        ...
        if provider in {"sif", "sellersprite"}:
            accepted_statuses = ["authenticated_mcp", "authenticated_web"]
        requirements.append(
            {
                ...
                **({"source_policy": MCP_ERROR_ONLY_POLICY, "entry_priority": ["mcp", "web"]} if provider in {"sif", "sellersprite"} else {}),
                "status": "pending",
                "task_id": None,
                "host": None,
                "dispatch_id": None,
...
def verify_copy_payload(manifest: dict[str, Any], path: Path) -> dict[str, Any]:
    ...
    if manifest.get("execution_controls_version") == "20260911":
        review = payload.get("bullet_review", [])
        if (not isinstance(review, list) or len(review) != len(bullets)
                or [item.get("bullet_number") for item in review] != list(range(1, len(bullets) + 1))):
            raise ValueError("Bullet review must cover every confirmed field exactly once")
        for item, text in zip(review, bullets):
            if item.get("text_sha256") != hashlib.sha256(text.encode("utf-8")).hexdigest():
                raise ValueError("Bullet review text changed")
            required = ("purchase_reason", "mechanism_or_fact", "user_action_or_context", "benefit", "review_note")
            if not all(isinstance(item.get(key), str) and item[key].strip() for key in required):
                raise ValueError("Bullet review needs reason, fact, usage, benefit and review note; no character minimum")
            if item.get("status") not in {"通过", "可优化", "质量缺口"}:
                raise ValueError("Bullet review status must distinguish soft quality from hard rules")
            refs = item.get("source_refs", [])
            if not refs or any(not ref.get("locator") or seen.get(str(Path(ref["path"]).resolve())) != ref.get("sha256") for ref in refs):
                raise ValueError("Bullet review needs locatable facts in the verified source locks")
    from runtime_recovery import enabled, verify_front_review
    if enabled(manifest):
        verify_front_review(payload, seen)
    return payload
...
def cmd_init(args: argparse.Namespace) -> int:
    ...
    manifest: dict[str, Any] = {
        ...
        "quality_execution": {"final_checks": "inline", "independent_qa": "not_requested", "p1": False},
        "writing_rules_version": WRITING_RULES_VERSION,
        "bullet_count_policy": BULLET_COUNT_POLICY,
        "run_id": run_id,
        "execution_controls_version": "20260911",
        "runtime_repairs_version": "20260914",
        "login_gate_policy": "per-stage-20260914",
...
def cmd_set_stage(args: argparse.Namespace) -> int:
    ...
    if stage == "final_qa" and new_status in {"running", "completed"} and manifest.get("quality_execution", {}).get("final_checks") == "inline":
        raise ValueError("Use finish-inline-checks; final_qa is an assembly receipt, not an independent task")
    ...
    if stage in LOGIN_GATED_STAGES and new_status in {"running", "completed"}:
        if "login_sessions" in manifest:
            if (manifest.get("login_gate_policy") != "per-stage-20260914"
...
def cmd_finish_inline_checks(args: argparse.Namespace) -> int:
    """Accept the assembler's complete Gate 7 receipt without a second QA dispatch."""
    path = Path(args.manifest).resolve()
    manifest = load_manifest(path)
    if manifest.get("quality_execution", {}).get("final_checks") != "inline":
        raise ValueError("Legacy Run quality routing must not be silently upgraded")
    verify_calibration_lock(manifest)
    verify_copy_lock(manifest)
    generation = manifest["stages"]["listing_generation"]
    if generation["status"] != "completed":
        raise ValueError("Final listing output must complete before assembly checks are accepted")
    receipt_path = Path(args.receipt).resolve()
    receipt = json.loads(receipt_path.read_text(encoding="utf-8"))
    if (receipt.get("run_id") != manifest["run_id"] or receipt.get("kind") != "assembly_self_check"
            or receipt.get("input_sha256") != manifest["input"]["sha256"]
            or receipt.get("copy_sha256") != manifest["copy_checkpoint"]["copy_sha256"]
            or receipt.get("independent_qa") != "not_executed" or receipt.get("p1") is not False):
        raise ValueError("Wrong inline check identity/type; cannot claim independent QA or P1")
    if receipt.get("checker_sha256") != sha256_file(Path(__file__)):
        raise ValueError("Inline checker version changed")
    rules = receipt.get("rule_locks", [])
    quality_contract = Path(__file__).resolve().parents[1] / "references" / "quality-gates.md"
    from runtime_recovery import enabled, quality_contract_bound
    quality_bound = any(Path(lock["path"]).resolve() == quality_contract for lock in rules)
    if not quality_bound and enabled(manifest):
        binding = receipt.get("quality_contract_binding", {})
        accepted = manifest.get("assembly_owner_binding", {})
        if (not accepted or binding.get("owner_manifest") != accepted.get("owner_manifest")
                or binding.get("main_manifest") != accepted.get("main_manifest")):
            raise ValueError("Quality contract binding must match the accepted dispatch package locks")
        quality_bound = quality_contract_bound(rules, quality_contract, receipt)
    if not rules or not quality_bound:
        raise ValueError("Current quality contract and owning rule locks required")
    for lock in rules:
        if sha256_file(Path(lock["path"])) != lock["sha256"]:
            raise ValueError("Owner rule changed after checks")
    outputs = generation.get("output_sha256", {})
    if not outputs or set(outputs) != set(generation.get("outputs", [])) or receipt.get("output_sha256") != outputs:
        raise ValueError("Receipt must bind every accepted final output")
    for filename, expected in outputs.items():
        if sha256_file(Path(filename)) != expected:
            raise ValueError("Final artifact changed after assembly checks")
    if manifest.get("execution_controls_version") == "20260911":
        from final_workbook_schema import check_xlsx
        finals = [Path(name) for name in outputs if Path(name).name == "10_Amazon Listing交付.xlsx"]
        if len(finals) != 1:
            raise ValueError("Exactly one fixed-name final 10 workbook required")
        layout_errors = check_xlsx(finals[0])
        if layout_errors:
            raise ValueError("Final layout rejected: " + "; ".join(layout_errors))
    checks = receipt.get("checks", {})
    if set(checks) != set(INLINE_CHECK_IDS):
        raise ValueError("Complete Gate 7 and writing-review coverage required")
    for check in checks.values():
        if check.get("status") != "pass" or not check.get("evidence"):
            raise ValueError("Failed or missing owner checks cannot be accepted")
        for record in check["evidence"]:
            if sha256_file(Path(record["path"])) != record["sha256"]:
                raise ValueError("Owner check evidence changed")
    if not isinstance(receipt.get("gaps"), list):
        raise ValueError("Explicit gaps list required")
    previous = manifest["stages"]["final_qa"]
    receipt_hash = sha256_file(receipt_path)
    if previous["status"] == "completed":
        if previous.get("output_sha256") != {str(receipt_path): receipt_hash}:
            raise ValueError("Do not replace accepted checks; reopen affected assembly first")
        return 0  # Identical notification is read-only.
    previous.update(status="completed", ended_at=now_iso(), outputs=[str(receipt_path)],
                    output_sha256={str(receipt_path): receipt_hash},
                    message="Assembly self-checks completed; independent QA not executed; P1=false")
    append_event(manifest, "inline_checks_accepted", receipt_sha256=receipt_hash)
    manifest["overall_status"] = derive_overall_status(manifest)
    atomic_write(path, manifest)
    print(json.dumps({"overall_status": manifest["overall_status"], "independent_qa": "not_executed", "p1": False}))
    return 0


def sif_session_evidence(manifest, args, path, *, fallback=False):
    """Validate non-sensitive current-owner observations, not service credentials."""
    path = Path(path).expanduser().resolve()
    if not path.is_relative_to(Path(manifest["directories"]["evidence"]).resolve()):
        raise ValueError("SIF evidence must remain inside this Run's local evidence directory")
    payload = json.loads(path.read_text(encoding="utf-8"))
    expected = {
        "schema": "listing-sif-web-fallback/v1" if fallback else "amazon-keyword-mcp-authentication/v1",
        "run_id": manifest["run_id"], "task_id": args.task_id, "host": args.host,
        "provider": "SIF", "entry_type": "mcp",
    }
    allowed = set(expected) | {"checked_at", "reason" if fallback else "authenticated"}
    if (not isinstance(payload, dict) or set(payload) - allowed
            or any(payload.get(key) != value for key, value in expected.items())):
        raise ValueError("SIF evidence schema, current Run/Task/host/provider or allowed fields mismatch")
    if fallback:
        if payload.get("reason") not in SIF_WEB_FALLBACK_REASONS:
            raise ValueError("SIF web fallback needs a supported MCP unavailability reason; wrong-site or unclear authority remains blocked")
    elif payload.get("authenticated") is not True or args.mcp_authenticated is not True:
        raise ValueError("SIF MCP requires a successful current-task authentication check")
    return {"path": str(path), "sha256": sha256_file(path), "bytes": path.stat().st_size}


def source_login_evidence(manifest, args, session, path, kind):
    """Current-owner MCP auth or error-only website login evidence; no secrets."""
    path = Path(path).expanduser().resolve()
    root = Path(manifest["directories"]["evidence"]).resolve()
    if not path.is_relative_to(root):
        raise ValueError("Source login evidence must remain inside this Run's evidence directory")
    data = json.loads(path.read_text(encoding="utf-8"))
    provider = {"sif": "SIF", "sellersprite": "SellerSprite"}[session["provider"]]
    expected = {"run_id": manifest["run_id"], "task_id": args.task_id, "host": args.host, "provider": provider}
    if kind in {"mcp", "web"}:
        expected.update(schema=f"amazon-keyword-{kind}-authentication/v1", entry_type=kind, authenticated=True)
        extras = {"checked_at"}
    elif kind == "error":
        expected.update(schema="listing-source-mcp-error/v1", entry_type="mcp", is_error=True)
        extras = {"checked_at", "error_code", "evidence"}
    else:
        expected.update(schema="listing-source-web-login-notice/v1", login_requested=True)
        extras = {"checked_at"}
    if (not isinstance(data, dict) or set(data) - (set(expected) | extras)
            or any(data.get(k) != v or (isinstance(v, bool) and data.get(k) is not v) for k, v in expected.items())):
        raise ValueError("Source login evidence requires exact schema/current Run/Task/host/provider and true status")
    if kind == "mcp" and args.mcp_authenticated is not True:
        raise ValueError("MCP requires a successful current-task authentication check")
    if kind == "error":
        if not isinstance(data.get("error_code"), str) or not data["error_code"].strip() or not data.get("evidence"):
            raise ValueError("Explicit MCP error code and raw evidence required; zero/incomplete data is not an error")
        if not isinstance(data["evidence"], list):
            raise ValueError("Raw error evidence must be a list")
        for lock in data["evidence"]:
            raw = Path(lock["path"]).resolve()
            if set(lock) != {"path", "sha256"} or not raw.is_relative_to(root) or sha256_file(raw) != lock["sha256"]:
                raise ValueError("Raw MCP error evidence path/hash mismatch")
    return {"path": str(path), "sha256": sha256_file(path), "bytes": path.stat().st_size}
...
def cmd_confirm_login_session(args: argparse.Namespace) -> int:
    ...
    mcp_primary = args.status == "authenticated_mcp"
    observed_domain = None if mcp_primary else normalize_domain(args.observed_domain)
    ...
    if session.get("source_policy") == MCP_ERROR_ONLY_POLICY:
        if manifest.get("source_policies") != SOURCE_POLICIES:
            raise ValueError("Source policy does not match this Run")
        evidence = source_login_evidence(manifest, args, session, evidence_path, "mcp" if mcp_primary else "web")
        if not mcp_primary:
            error_path = getattr(args, "mcp_error_evidence", None)
            notice_path = getattr(args, "web_login_notice", None)
            if not error_path or not notice_path:
                raise ValueError("MCP-first website fallback requires explicit MCP error evidence and user login notice")
            fallback_evidence = {
                "error": source_login_evidence(manifest, args, session, error_path, "error"),
                "login_notice": source_login_evidence(manifest, args, session, notice_path, "notice"),
            }
    elif session.get("source_policy") == SIF_SOURCE_POLICY:
        if manifest.get("source_policies", {}).get("sif_competitor") != SIF_SOURCE_POLICY:
            raise ValueError("SIF source policy does not match this Run")
        if mcp_primary:
            evidence = sif_session_evidence(manifest, args, evidence_path)
        elif args.status == "authenticated_web":
            fallback_path = getattr(args, "sif_fallback_evidence", None)
            if not fallback_path:
                raise ValueError("SIF is MCP-first; web fallback requires current-task MCP unavailability evidence")
            fallback_evidence = sif_session_evidence(manifest, args, fallback_path, fallback=True)
    elif mcp_primary:
        raise ValueError("Historical SIF Run cannot silently switch to MCP-first")
    receipt = {
        "status": args.status,
        ...
        **({"sif_fallback_evidence": fallback_evidence} if session.get("source_policy") == SIF_SOURCE_POLICY else {}),
        **({"source_fallback_evidence": fallback_evidence} if session.get("source_policy") == MCP_ERROR_ONLY_POLICY else {}),
    }
...
def cmd_confirm_copy(args: argparse.Namespace) -> int:
    ...
    manifest["stages"]["copy_checkpoint"].update({
        "status": "completed", "started_at": now_iso(), "ended_at": now_iso(),
        "message": f"Copy confirmation recorded by {args.confirmed_by} for all {len(gate['confirmed_scope'])} fields",
        "outputs": [str(draft)], "output_sha256": {str(draft): draft_hash},
    })
    append_event(manifest, "listing_copy_confirmed", **gate)
    manifest["overall_status"] = derive_overall_status(manifest)
    atomic_write(path, manifest)
    print(json.dumps({k: gate[k] for k in ("status", "copy_file", "copy_sha256")}, ensure_ascii=False))
    return 0
...
def cmd_update_progress(args: argparse.Namespace) -> int:
    """Checkpoint metadata only: never advance gates, outputs or completion."""
    path = Path(args.manifest).resolve()
    manifest = load_manifest(path)
    record = manifest["stages"][args.stage]
    if record["status"] != "running" or not args.message.strip():
        raise ValueError("Progress updates require a running stage and nonempty message")
    evidence = Path(args.evidence).resolve()
    if not evidence.is_file() or not evidence.is_relative_to(path.parent):
        raise ValueError("Progress evidence must be an existing file inside this Run")
    record["progress"] = {"message": args.message, "at": now_iso(),
                          "evidence": {"path": str(evidence), "sha256": sha256_file(evidence)}}
    append_event(manifest, "stage_progress_observed", stage=args.stage, progress=record["progress"])
    atomic_write(path, manifest)
    print(json.dumps({"stage": args.stage, "status": "running", "progress_saved": True}))
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    progress_parser = subparsers.add_parser("update-progress", help="Save a running checkpoint without changing gates")
    progress_parser.add_argument("--manifest", required=True)
    progress_parser.add_argument("--stage", required=True, choices=STAGES)
    progress_parser.add_argument("--message", required=True)
    progress_parser.add_argument("--evidence", required=True)
    progress_parser.set_defaults(func=cmd_update_progress)

    inline_parser = subparsers.add_parser("finish-inline-checks", help="Accept existing assembly checks without independent QA")
    inline_parser.add_argument("--manifest", required=True)
    inline_parser.add_argument("--receipt", required=True)
    inline_parser.set_defaults(func=cmd_finish_inline_checks)

    session_parser = subparsers.add_parser(
        "confirm-login-session", help="Register one exact owner-task web or SIF MCP receipt"
    )
    ...
    session_parser.add_argument(
        "--status",
        required=True,
        choices=["authenticated_web", "authenticated_mcp", "user_approved_same_provider_mcp"],
    )
    session_parser.add_argument("--observed-domain", help="Required for website receipts; omitted for SIF authenticated_mcp")
    ...
    session_parser.add_argument("--sif-fallback-evidence", help="Current-Run non-sensitive MCP unavailability evidence before SIF web fallback")
    session_parser.add_argument("--mcp-error-evidence", help="Current-owner explicit MCP error and raw evidence; required before website fallback")
    session_parser.add_argument("--web-login-notice", help="Current-owner evidence that the user was asked to log in to the fallback website")
