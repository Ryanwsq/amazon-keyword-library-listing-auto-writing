...
| 0.5 | login_gate | 各实际拥有任务 + 本Skill状态脚本 | 锁定站点、`登录准备`与固定八会话矩阵 | 六个Amazon逐任务手动登录回执 + SIF与卖家精灵MCP鉴权回执（仅明确报错后提示用户登录官网备用） | `confirm-login-session`逐条登记，`finalize-login-gate`收口；失效只阻断直接依赖阶段 |
...
| 13 | final_qa | 装配拥有任务自检，本Skill验收回执 | 09/10 + 全部证据 | 原阶段键下登记内联装配检查，不另派独立QA | Gate 7或证据不闭合不得完成 |
...
- 每次状态变化/恢复和等待前按[执行减负合同](execution-efficiency.md)调用全图扫描并对账全部实际派发。扫描不代替原门，不因某一路失败暂停无关就绪任务。新Run final_qa为装配内检查回执，不再独立追加一轮QA；旧Run原合同不变。

- 输入预检完成后，关键词阶段可以立即向 Amazon 关键词主任务下发锁定输入，并与阶段 1、2、4 并行等待；关键词项目回传后再串行进入本项目 SKU 终筛。
- `full_pipeline`在输入与marketplace锁定后立即统一引导八会话准备；新Run的per-stage-20260914按本分支会话及真实依赖就绪即启动，总门保留为汇总，旧Run按原锁。关键词主任务先接收输入和复用请求，不由此豁免各源会话。`downstream_intake`不为已封存01–05重复登录，但新的关键词采集仍须完成Amazon拥有任务网页会话及SIF/卖家精灵拥有任务MCP鉴权，官网备用先按合同取证再登录。
