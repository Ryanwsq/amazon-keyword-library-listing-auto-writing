...
固定三行服务：`SIF`、`卖家精灵`、`Amazon`。SIF和卖家精灵的执行默认MCP优先，可填写`MCP鉴权`，账户别名/凭据引用可空，密钥只留本机安全配置。既有模板或输入填写的其他方式仅为官网备用登录偏好，不覆盖MCP优先、不要求先网页登录；本批不改变表头或模板布局。两者仅明确MCP报错后由主任务要求用户登录官网，可选择`用户手动`（账户别名/凭据引用可空），或选择`浏览器已保存凭据`、`本机密码管理器`（必须填写账户别名及本机非敏感凭据引用，例如浏览器配置名或条目名）。Amazon固定为`用户手动`，账户别名和凭据引用不填写。选择手动不等于已登录，仍须本Run的实际拥有任务独立回执。
...
完整输入与站点锁定后，`full_pipeline`按[任务会话登录合同](login-session-contract.md)先进入固定八会话登录门。主任务立即引导用户准备全部会话：SIF任务先取得MCP鉴权证明，仅需要官网备用时再登录；卖家精灵按本Sheet选择用户手动或使用本机已保存凭据，六个Amazon拥有任务由用户逐任务手动登录。未全部回执前不得发起查询、提问、导出或联想；主任务或其他会话的Cookie不能代替拥有任务。
...
非标准旧表不能按列号猜值：站点列被附加关键词字段挤到末尾、旧名`station`等情况，先核对显式表头及用户含义；有唯一映射时另存本Run规范化副本和逐字段来源映射/原新哈希，保留全部额外字段用于附加请求。原件不改；重复、冲突或未知站点一次性回询。规范化副本仍须通过本校验器，不能把带错误的预检结果改写为通过。
...
每个实际拥有任务使用`pipeline_state.py confirm-login-session`登记独立回执，字段至少含`session-key/task-id/host/dispatch-id/status/evidence-file`；网页另含`observed-domain`，SIF首选MCP为`authenticated_mcp`且须当前任务鉴权证据，不伪造网页域名；Amazon再带锁定邮编，需要购物助手的角色再带Alexa或Rufus。八份回执齐全后使用`finalize-login-gate`完成总门。任一会话失效时使用`invalidate-login-session`，只阻断其直接依赖阶段。完整矩阵、SIF MCP优先/官网备用和证据边界见[任务会话登录合同](login-session-contract.md)。
