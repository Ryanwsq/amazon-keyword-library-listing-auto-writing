...
- Run锁定站点的Amazon Listing与图片核验属于执行动作，不是第三个用户输入；US使用amazon.com/10001，DE使用amazon.de/80539。输入与站点锁定后，主任务在登录准备阶段引导用户在本任务Amazon会话手动登录；必须取得本Task/host对应`listing:tag-priority:amazon`的`authenticated_web`回执后才核验，其他任务登录态不能代替。进入其他站点时停止并通知用户介入。
...
- `未执行`、`身份异常`、`未抓取到`、页面不可访问和技术停止都不是 0；`⚠️/❗`图标必须附文字状态与影响范围，不能单独阻断整条流程。
...
- 对每个有效竞品核验Run锁定站点Listing的Title、Image 2–4、Bullet 1–2、A+核心、后部副图、Bullet 3–5、A+其他。
