...
class MarketplaceRouteTests(unittest.TestCase):
    ...
    def confirm_session(
        ...
        *,
        ...
        if status is None:
            status = "authenticated_mcp" if session.get("source_policy") == pipeline_state.SIF_SOURCE_POLICY else "authenticated_web"
        ...
        identity = {"run_id": manifest["run_id"], "task_id": task_id or f"task-{index}", "host": "test-host", "provider": "SIF", "entry_type": "mcp"}
    ...
    def test_mcp_input_choice_is_sif_only(self):
        errors = []
        validate_input.validate_login_profile("SIF", "", "", "MCP鉴权", 2, errors)
        self.assertEqual([], errors)
        for provider in ("Amazon", "卖家精灵"):
            errors = []
    ...
    def test_sif_web_is_only_an_evidenced_fallback(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.confirm_session(path, sif, 7, status="authenticated_web", fallback_reason="mcp_unavailable")
            saved = pipeline_state.find_login_session(json.loads(path.read_text()), sif["session_key"])
            self.assertEqual("authenticated_web", saved["status"])
            self.assertTrue(saved["sif_fallback_evidence"]["sha256"])

    def test_other_seven_sessions_stay_web_only(self):
        matrix = pipeline_state.build_login_sessions("Amazon-US")
        others = [x for x in matrix["requirements"] if x["provider"] != "sif"]
