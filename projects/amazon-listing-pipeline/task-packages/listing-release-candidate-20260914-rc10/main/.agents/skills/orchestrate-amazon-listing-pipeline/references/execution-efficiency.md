...
2026-09-11维护补充：`init`为新Run登记`execution_controls_version=20260911`，不改历史Run。新增控制只将既有入口和输出合同落实为可测试准备/装配检查；不是后台常驻调度器或浏览器故障修复。

2026-09-14新Run另锁`runtime_repairs_version=20260914`。完整读取[当前任务恢复合同](runtime-recovery-contract.md)：分支登录、唯一活动目标、实际运行态观察、当前派发准入、正文前段/预算自检和跨包质量合同绑定。下文旧总门叙述仅适用于未锁新登录策略的历史Run。
...
`full_pipeline`新Run在各自登录及依赖齐备后同波次派商品审计、五维洞察和痛点频率；关键词主任务可先接收锁定输入做编排，实际采集仍按其各源门。历史Run保留原总登录门。五维完成即派标签，不等关键词；关键词正式回执通过原核验并由Listing主任务发READY后即派SKU，不等Alexa其他支线。02/03与04/05的接收整理仍保留，但只核验/登记，不重写源表。08与09在07确认后按原条件并行；正文仍等待09计划，最终装配仍等待正文确认，不把有依赖任务强行并行。同一个实际浏览器会话不并发操作。
...
- 新版`full_pipeline`须有`keyword_request={path,sha256}`，指向按[关键词交接合同](keyword-handoff-contract.md)生成的当前输入/站点/复用优先请求。缺失时只阻断关键词支线，其他就绪支线继续；测试不自动禁用复用。实际复用资格仍由关键词主任务判断。
...
- 实际工具返回可原样保存MCP的`content[].text` JSON或`structuredContent`包装；扫描器解析真实threadId，拒绝失败、冲突或不符目标的响应，不能拿拟派名单充当发送回执。首次接收、恢复/压缩后首个业务读写前重新核对活动dispatch/Run/产品/允许来源和输出根；app cwd、业务执行根和Git根分别核验，旧聊天不是当前产品变更授权。
...
新固定布局工具见[装配阶段](stage-listing-generation.md)及[布局数据](final-workbook-schema.json)：只固化原14Sheet/表头和公开允许的审计列，未把全部业务写作实现为通用builder。自建表先检查行矩阵/基础XLSX，再渲染及搬源表；最终10再次核对。`finish-inline-checks`对新控制版本强制检查固定文件名及布局，仍保留全部原检查ID；结构通过不代表语义/可视化通过。

主任务收到完整封存回执再镜像，不以临时文件存在或任务active报装配完成。进度区分`已发送/已接收/正在生成/布局预检/渲染与保真/正式回执已收/已交付`；只依据真实回执/产物阶段报告，不在渲染或返工未闭合时反复声称“只差几分钟”。相同哈希已检查的预览可引用，改动范围重新检查；不重复派独立QA。
