...
6. 锁定输入并通过适用登录门后，按 `stage-keywords.md` 将关键词所需输入下发给 `Amazon关键词词库｜主任务｜main`；等待其将完整总工作簿回传至本项目 `副线程｜SKU可用关键词库`，主线程登记并核验这次跨项目交接。
7. 按 `pipeline-map.md` 执行或接收其他数据阶段；每阶段开始和结束都更新 `run-manifest.json`。
