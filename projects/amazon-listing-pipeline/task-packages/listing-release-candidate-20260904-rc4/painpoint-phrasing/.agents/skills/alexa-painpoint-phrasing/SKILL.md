...
Accept only the `.xlsx` output created by `alexa-painpoint-frequency`. It must contain both required sheets and columns. The current Run must also provide a required `marketplace`: `Amazon-US` or `Amazon-DE`. US uses amazon.com/10001/Alexa/English; DE uses amazon.de/80539/Rufus/German. It must match the upstream pain-point Run; missing or conflicting marketplace blocks execution.
...
The shopping assistant needs a short cue in the Run prompt language to replace `[PAIN POINT]` in the fixed questions.

1. For Amazon-US, prefer upstream English `Supporting wording`; for Amazon-DE, prefer upstream German supporting wording when it clearly represents the standardized pain point.
2. Otherwise translate `标准化痛点` into a neutral concept in the locked prompt language.
...
Use the authenticated shopping-assistant browser for the Run marketplace. During the post-input login-preparation step, open the matching Amazon site in this task and wait for the user to log in manually. Do not ask questions until this exact Task/host returns an `authenticated_web` receipt for `listing:painpoint-phrasing:amazon`; another task's login cannot satisfy it. A different marketplace is `marketplace_mismatch` and requires user intervention.

For Amazon-DE, use Rufus and record `context_independence=not_available`. Refresh may preserve context, so do not repeatedly refresh or claim it cleared the chat. Both German prompts contain the ASIN. If the Rufus panel is already visible, type directly; clicking the toggle again can hide it. For each deduplicated `标准化痛点 + ASIN` pair:

1. Open or bind Alexa to the exact ASIN and confirm the product matches.
2. Insert the cue in the locked prompt language into Q1 and Q2.
3. Ask both questions in order and preserve the complete raw answers.
4. Keep ASIN sessions separate on Amazon-US. On Amazon-DE, where Rufus cannot reliably create a new chat, keep each prompt self-contained with the ASIN and record the context limitation.
...
Use these exact questions with the German cue; do not translate or rewrite them during the Run.

- Q1: `Wie würden Käufer das Problem „[PAIN POINT]“ beim Amazon-Artikel {ASIN} in lockerer Alltagssprache beschreiben? Gib nur kurze, natürliche Ausdrücke aus, einen pro Zeile. Erkläre und bewerte nichts und verwende keine Fach- oder Werbesprache.`
- Q2: `Wie würden Käufer vor dem Kauf des Amazon-Artikels {ASIN} auf natürliche Weise nach „[PAIN POINT]“ fragen? Gib nur kurze, umgangssprachliche Fragen aus, eine pro Zeile. Erkläre und bewerte nichts und verwende keine Fach- oder Werbesprache.`
