...
- `marketplace` is required and must be `Amazon-US` or `Amazon-DE`. US uses amazon.com/10001/Alexa/English; DE uses amazon.de/80539/Rufus/German. Never silently default to US.
...
- If an ASIN is invalid, unavailable on the Run-locked marketplace, resolves to the wrong product, or cannot be queried completely, record its processing status in `痛点明细` and exclude it from the frequency denominator.

## Alexa access

Use the available authenticated Alexa for Shopping browser or app connection. For each ASIN:

During the post-input login-preparation step, open the matching Amazon site in this task and wait for the user to log in manually. Do not ask any question until this exact Task/host returns an `authenticated_web` receipt for `listing:painpoint-frequency:amazon`; another task's login cannot satisfy it. If the page enters another Amazon marketplace, stop, record `marketplace_mismatch`, and request user intervention.

For Amazon-DE, use Rufus. Rufus has no reliable `New Chat`, and refresh may preserve context: record `context_independence=not_available`, do not repeatedly refresh, and do not block for this limitation. Every German prompt includes the ASIN. If the Rufus panel is already visible, type directly; clicking its toggle again can hide it.
...
3. Keep product sessions separate on Amazon-US. On Amazon-DE, use self-contained ASIN prompts and record the accepted Rufus context limitation.
...
### Amazon-DE — feste deutsche Fragen

Use the following exact German prompts for Rufus. Do not translate or rewrite them during the Run. The required response format is `Problem | Quelle | Belegformulierung`; the empty result is `Nicht gefunden`.

1. `Welche konkreten Probleme, Nachteile oder Einschränkungen berichten Käufer in Kundenrezensionen ausdrücklich zum Amazon-Artikel {ASIN}? Nenne ein Problem pro Zeile im Format: Problem | Quelle | Belegformulierung. Leite nichts ab und behandle fehlende Informationen nicht als Problem. Falls nichts belegt ist, antworte „Nicht gefunden“.`
2. `Welche Probleme, Bedenken oder Einschränkungen sprechen Käufer in Kundenfragen und -antworten ausdrücklich zum Amazon-Artikel {ASIN} an? Nenne ein Bedenken pro Zeile im Format: Problem | Quelle | Belegformulierung. Leite nichts hinzu. Falls nichts belegt ist, antworte „Nicht gefunden“.`
3. `Erwähnen Kundenrezensionen oder Kundenfragen und -antworten ausdrücklich Probleme mit Funktionen, Leistung oder dem erwarteten Ergebnis des Amazon-Artikels {ASIN}? Nenne nur belegte Probleme, eines pro Zeile: Problem | Quelle | Belegformulierung. Falls nichts gefunden wird, antworte „Nicht gefunden“.`
4. `Erwähnen Kundenrezensionen oder Kundenfragen und -antworten ausdrücklich Probleme mit Material, Verarbeitung, Schäden, Verschleiß, Haltbarkeit oder Zuverlässigkeit des Amazon-Artikels {ASIN}? Nenne nur belegte Probleme, eines pro Zeile: Problem | Quelle | Belegformulierung. Falls nichts gefunden wird, antworte „Nicht gefunden“.`
5. `Erwähnen Kundenrezensionen oder Kundenfragen und -antworten ausdrücklich Schwierigkeiten bei Montage, Einrichtung, Bedienung, Einstellung, Reinigung, Wartung, Lagerung oder Transport des Amazon-Artikels {ASIN}? Nenne nur belegte Probleme, eines pro Zeile: Problem | Quelle | Belegformulierung. Falls nichts gefunden wird, antworte „Nicht gefunden“.`
6. `Erwähnen Kundenrezensionen oder Kundenfragen und -antworten ausdrücklich Probleme mit Größe, Passform, Kapazität, Kompatibilität, Platzbedarf oder Eignung des Amazon-Artikels {ASIN} für bestimmte Nutzer oder Situationen? Nenne nur belegte Probleme, eines pro Zeile: Problem | Quelle | Belegformulierung. Falls nichts gefunden wird, antworte „Nicht gefunden“.`
7. `Beschreiben Kundenrezensionen oder Kundenfragen und -antworten ausdrücklich Situationen, in denen der Amazon-Artikel {ASIN} schwer nutzbar, ungeeignet oder nicht den Erwartungen entsprechend ist? Nenne nur belegte Probleme, eines pro Zeile: Problem | Quelle | Belegformulierung. Spekuliere nicht. Falls nichts gefunden wird, antworte „Nicht gefunden“.`
