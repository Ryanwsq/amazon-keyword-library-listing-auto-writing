...
def plan(manifest, board, mode):
    ...
    def add(node, role, dependencies=(), *, gate=None, terminal_dependencies=False, status=None):
        ...
        row["stage_key"] = key
        if node in invalid:
            row.update(action="repair_lock", reason=invalid[node])
            ...
            if any(proof.get(k) != v for k, v in {"run_id": manifest["run_id"], "node": node,
                ...
                if delivery in {"sent", "accepted", "running"}:
                    if proof.get("response", {}).get("threadId") != proof["task_id"] and not proof.get("tool_call_id"):
    ...
    add("preflight", "main")
    login_ready = stages.get("login_gate", {}).get("status") == "completed"
    ...
    add("keyword_library", "keyword-main", ("preflight",),
        gate=(login_ready and not state.missing_login_sessions(manifest, "keywords")
