...
本模块收到明确分发的角色、Run/SKU、规则版本、输入路径与SHA-256和启动授权后。输入：合格04中已有的标准化痛点与ASIN配对、`marketplace`及派生路由、本Task/host对应`listing:painpoint-phrasing:amazon`的登录回执。只读初始化时不启动业务。
