...
结构版本：`listing-structure-2`；复核日期：2026-09-04。
...
2026-09-04候选rc2经`AKW-DE-VALIDATION-FIXES-20260904-01`授权，补充输入站点锁、Amazon-US/Amazon-DE路由、统一网页登录门、德国站Rufus固定德语问题及不可独立上下文例外、站点专属硬规则和告警/阻断语义。

2026-09-04候选rc3经`AKW-TASK-SESSION-LOGIN-20260904-01`授权，在rc2不可变前序上增加无秘密`登录准备`Sheet、固定八Task/host会话矩阵、逐会话回执/失效与分支阻断；Amazon由用户逐任务手动登录，SIF/卖家精灵按非敏感引用使用本机已保存凭据。旧候选与历史Run不可变；本批只进行结构和机械回归，`business_run=not_executed`、`P1=not_executed`，下一次规则/Skill能力验证必须使用`test-validation + full-regression`。

2026-09-04候选rc4经`AKW-TASK-SESSION-LOGIN-20260904-02`授权，在rc3不可变前序上补齐必填marketplace与八会话初始化回归；业务合同、字段和P1状态不再扩大。
