...
| bullet_review | 仅execution_controls_version=20260911新Run必填；每条对应一条记录，按1–N排序，字段见下文；将已有软性自检绑定实际正文，不增加字数门或第二个QA任务 |
...
新控制版本的`bullet_review`每项含`bullet_number、text_sha256`（该条UTF-8正文SHA-256）、`purchase_reason、mechanism_or_fact、user_action_or_context、benefit、review_note、status、source_refs`。事实指针含`path/sha256/locator`且属于本草案已验证的source_locks；status仅`通过/可优化/质量缺口`。与草案同一JSON保存，改字后重审对应项，不要求逐次另建整套QA文件。

脚本只验证覆盖、正文绑定及事实定位，不能判断英语是否自然。拥有任务必须按软性知识库逐项核对信息增量、动作/场景和收益是否充分；缺事实要具体写缺什么，不能把单纯的展开不足伪装成“事实不足”而反复压短。先完整读取当前用户补充，不凭旧Run或其他产品模板补事实。普通按摩功能不得自行强化成医疗/持久缓解效果。用户明确的新表达/事实只在当前Run锁定，不转成所有产品通用规则。

`confirm-copy`现在同时登记阶段output_sha256，完整确认后不再需要人工补哈希才能派发装配。记录使用实际confirmed_by，不把主任务登记冒称为用户逐字审阅；默认双确认门不变，不能把“继续写”解释为确认或一般性代审批。
