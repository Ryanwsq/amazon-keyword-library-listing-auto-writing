...
2026-09-07新增Run记录quality_execution（final_checks=inline、independent_qa=not_requested、p1=false）。final_qa阶段键保留但只登记装配自身检查回执，不表示独立QA。全量原检查/哈希及回执格式见[执行减负合同](execution-efficiency.md)；没有新增业务Sheet或改变原表字段，历史Run不迁移。
