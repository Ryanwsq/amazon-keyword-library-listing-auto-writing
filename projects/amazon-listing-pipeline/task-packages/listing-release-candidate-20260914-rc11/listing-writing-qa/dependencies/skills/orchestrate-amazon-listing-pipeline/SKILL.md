...
2026-09-14维护：新Run完整读取[当前任务恢复合同](references/runtime-recovery-contract.md)，执行分支登录门、唯一活动目标、真实运行态对账和当前派发准入；历史Run不热升级。准入脚本不是全局工具隔离，未实测不宣称已杜绝串任务。

2026-09-11 SIF入口顺序更新：竞品反查为SIF MCP优先、SIF官网备用，其他来源优先级不变。新Run在源策略中锁定该顺序；SIF MCP不以前置网页登录或逐Run例外批准为条件，仍须本Task/host鉴权。详见[任务会话登录合同](references/login-session-contract.md)。

2026-09-11执行修复：新Run按[执行减负合同](references/execution-efficiency.md)锁定执行控制版本；关键词派发先登记复用优先请求；SIF/卖家精灵可选用户手动登录但不豁免会话证据；正文逐条自检随草案回传；最终装配先校验固定表头再渲染/搬表，正式回执后才能交付。业务判断、词源、字段与双确认门不变，历史Run和角色包不迁移。
...
6. [references/execution-efficiency.md](references/execution-efficiency.md)：全图防漏派、固定输出复用和装配内检查。
...
4. 使用 `scripts/pipeline_state.py init --product-asin <已预检核对的本品ASIN> --marketplace <Amazon-US|Amazon-DE>` 创建新 Run、锁定输入副本或封存包登记、保存全部输入 SHA-256、`marketplace`路由与产品身份；旧Run只读恢复，不重新初始化。
5. `full_pipeline`锁定输入和站点后，完整读取[任务会话登录合同](references/login-session-contract.md)。立即把固定八会话矩阵下发到实际拥有任务并引导用户完成登录准备：SIF拥有任务先验证MCP鉴权，官网仅在拥有合同允许的MCP不可用/不完整分支准备登录；卖家精灵仍按输入表选择用户手动或非敏感已保存凭据；六个Amazon拥有任务必须由用户逐任务手动登录。各任务用`scripts/pipeline_state.py confirm-login-session`独立登记当前Task/host/dispatch/域名/邮编/助手/本地证据回执，八份齐全后运行`finalize-login-gate`汇总。新Run按per-stage-20260914策略，分支自身会话及数据依赖齐备立即派发，不等待无关登录；旧Run仍保留总门要求。单个会话失效只阻断其依赖阶段并要求重登。密码、验证码、Cookie、令牌和真实任务绑定不进入工作簿、聊天、Run公开包或Git。`downstream_intake`不重跑已封存网页采集，但新的关键词采集仍受对应会话门约束。
6. 锁定输入并通过适用登录门后，按 `stage-keywords.md` 将关键词所需输入下发给 `Amazon关键词词库｜主任务｜main`并登记实际发送回执；此时不阻塞等待该支线，立即进入步骤7派发其他就绪数据任务。完整关键词总表回传后按原合同核验并发本Run READY，立即派SKU终筛，不等其他支线。
7. 按 `pipeline-map.md` 并行派发或接收其他数据阶段。每次输入/登录/阶段状态验收、解阻、恢复和进入等待前运行`scripts/dispatch_plan.py`，列全全部就绪任务并核对实际派发；存在漏派、待验收或未查清送达时不得宣称已全部启动。状态变化仍写run-manifest，失败只影响依赖后代，两个接收整理角色不遗漏。完整同波次送达后按各任务最新cursor同时等待，不反复读全文。
8. 保留每个依赖项目和 Skill 的原始输出，再复制为 `output-contract.md` 中的固定编号文件；不得手工改写上游状态以迎合下游。
9. 汇总商品事实、参数优势、标签优先级、痛点、关键词和待决规则，生成 `07_核心卖点决策包.xlsx`。该文件同时承担“信息校准与初步排布决策包”的职责，必须覆盖七类确认对象：标签排序、全部卖点及优劣势、痛点与样本限制、P0 选择、主图 1–9 初步排布、A+ 1–7 初步排布及动态挤压、ST 去重范围等未决规则。
10. 在聊天中按七类对象展示摘要和初步排布，将 Run 置为 `WAITING_HUMAN_CONFIRMATION` 并停止。此时允许展示方案预览，但不得生成任何最终 Listing 文案或把主图/A+方案标为已锁定。
11. 用户可以逐项确认、修改、否决或要求重排。P0、标签排序或卖点优先级发生变化时，必须重新计算受影响的主图/A+初步排布并再次展示。只有七类对象全部锁定后，才使用 `pipeline_state.py confirm` 固化完整校准文件及其 SHA-256。
12. 完整07确认后，运行必要08、09计划映射及Title/Highlights/五点草案；使用新标题知识库，可参考直接竞品写法，关键词只从06取。调用写作Skill时须落实三个字段的尺寸表达判断及IH的相关部件合讲/同收益取舍；具体规则留在知识库，不能由总控另造事实或排序。
13. 主线程与用户逐项讨论整套正文；未确认置WAITING_COPY_CONFIRMATION并停止最终装配。确认后用pipeline_state.py confirm-copy封存草案JSON及来源哈希。
14. 根据已确认正文生成最终ST，回填09实际覆盖，再按确认排布和14Sheet展示合同装配10，完成规则和来源保真校验；不能静默改正文。四张原表按[原表保真装配](references/preserved-sheet-transfer.md)执行用户授权的OOXML底层复制：正文/展示表仍由表格工具生成，原表不重算，使用scripts/copy_preserved_sheets.py并验证公式缓存、图表、样式与关系闭包；失败不降级成值复制。
15. 新建schema2.0仍用双门/14Sheet，另锁定writing_rules_version=v2.2及bullet_count_policy=coverage_based_5_to_6；历史1.0及无该写法版本的2.0保持原合同，不静默升级。已授权局部改稿另存草案，不原地改变旧Run。交付最终工作簿和简明运行摘要。
16. 新Run不另派独立重复QA。写作/装配拥有任务完成质量合同Gate 7和软性自检，以同一输出/证据生成assembly_self_check回执；主任务使用pipeline_state.py finish-inline-checks收口。final_qa旧键只记录装配检查，不代表独立QA；独立QA未执行和P1=false如实保留。完整字段、事实、来源保真、公式/图表及首次必要渲染不减少。

## 运行状态

固定阶段键：

`preflight`、`login_gate`、`product_audit`、`market_insights`、`tag_priority`、`pain_points`、`keywords`、`selling_point_decision`、`human_checkpoint`、`painpoint_phrasing`、`keyword_allocation`、`listing_draft`、`copy_checkpoint`、`listing_generation`、`final_qa`。
