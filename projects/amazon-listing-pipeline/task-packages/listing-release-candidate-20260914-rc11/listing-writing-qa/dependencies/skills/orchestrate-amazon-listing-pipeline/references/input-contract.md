...
固定三行服务：`SIF`、`卖家精灵`、`Amazon`。SIF默认`MCP鉴权`，账户别名/凭据引用可空，密钥只留本机安全配置；其他方式仅为官网备用登录偏好，不覆盖MCP优先。SIF备用与卖家精灵可选择`用户手动`（账户别名/凭据引用可空），或选择`浏览器已保存凭据`、`本机密码管理器`（必须填写账户别名及本机非敏感凭据引用，例如浏览器配置名或条目名）。Amazon固定为`用户手动`，账户别名和凭据引用不填写。选择手动不等于已登录，仍须本Run的实际拥有任务独立回执。
