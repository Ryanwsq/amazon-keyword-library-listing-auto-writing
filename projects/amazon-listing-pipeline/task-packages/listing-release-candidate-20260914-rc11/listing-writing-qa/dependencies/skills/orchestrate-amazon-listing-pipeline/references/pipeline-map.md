...
| 0.5 | login_gate | 各实际拥有任务 + 本Skill状态脚本 | 锁定站点、`登录准备`与固定八会话矩阵 | 六个Amazon逐任务手动登录回执 + SIF MCP鉴权回执（官网仅备用）+ 卖家精灵网页回执 | `confirm-login-session`逐条登记，`finalize-login-gate`收口；失效只阻断直接依赖阶段 |
...
- `full_pipeline`在输入与marketplace锁定后立即统一引导八会话准备；新Run的per-stage-20260914按本分支会话及真实依赖就绪即启动，总门保留为汇总，旧Run按原锁。关键词主任务先接收输入和复用请求，不由此豁免各源会话。`downstream_intake`不为已封存01–05重复登录，但新的关键词采集仍须完成Amazon/卖家精灵拥有网页会话及SIF拥有任务MCP鉴权，官网备用先按合同取证再登录。
