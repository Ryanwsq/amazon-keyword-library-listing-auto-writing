...
from runtime_recovery import enabled, active_owner, current_activity
...
def plan(manifest, board, mode):
    ...
    def add(node, role, dependencies=(), *, gate=None, terminal_dependencies=False, status=None):
        ...
        owner = None
        binding_error = None
        if enabled(manifest) and role != "main" and current not in {"completed", "skipped"} and not blocked and gate is not False:
            try:
                owner = active_owner(board, role)
                row["target"] = {k: owner[k] for k in ("task_id", "host", "business_cwd", "package_manifest_sha256")}
                signature["owner"] = row["target"]
                key = digest(signature)
                row["stage_key"] = key
        ...
        if node in invalid:
            ...
            row.update(action="blocked", reason="own session or required gate is not ready")
        elif binding_error:
            row.update(action="bind_owner", reason=binding_error)
        elif node in jobs:
            ...
            if any(proof.get(k) != v for k, v in {"run_id": manifest["run_id"], "node": node,
                ...
                if delivery in {"sent", "accepted", "running"}:
                    ...
                    if enabled(manifest) and role != "main":
                        action, reason = current_activity(manifest, board, node, proof, owner)
                        row.update(action=action, reason=reason, dispatch_id=proof["dispatch_id"])
                    else:
    ...
    per_stage_login = (manifest.get("login_gate_policy") == "per-stage-20260914"
                       and isinstance(manifest.get("login_sessions"), dict))
    login_ready = per_stage_login or (stages.get("login_gate", {}).get("status") == "completed" and "login_gate" not in invalid)
    ...
    add("keyword_library", "keyword-main", ("preflight",),
        gate=(login_ready and (per_stage_login or not state.missing_login_sessions(manifest, "keywords")) and (not entry_required or bool(entry_hash))
