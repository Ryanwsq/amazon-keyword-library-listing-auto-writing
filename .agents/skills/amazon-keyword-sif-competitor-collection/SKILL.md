...
3. 在第一次查询前，先在长期副任务的内置浏览器验证SIF官网已登录。未登录时状态进入`awaiting_login`，暂停并只向主任务回传登录状态、入口和受影响ASIN；由主任务提示用户登录，不得由本副任务直接向用户发问，也不得仅因当前未登录而切换MCP。登录验证通过后，对每个获准竞品 ASIN 分别通过官网查询同一最近30天口径并使用完整官方导出，最多300条，不叠加7天窗口。
