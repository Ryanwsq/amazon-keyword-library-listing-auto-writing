...
每个分类建立三列块：`关键词、月搜索量、竞争性强度`。上半区依次放F1–F5五个流量层块，共15列。下半区按最终总表动态语义列顺序展开`语义维度:标签值`块，每行横向四块；空块不输出。
...
唯一总manifest记录：交付身份；各模块版本；相对文件路径、类型、大小、SHA-256和状态；第一板块三来源/机械词池闭环；第二板块三去向；分类/词频/竞争/趋势/否词人口；最终七Sheet顺序/尺寸/图表/公式；唯一问题文档；QA状态；21项门结果。不得写接口正文、凭据、任务ID或绝对路径。
...
16. 最终工作簿恰好七个可见Sheet、顺序一致、无隐藏过程Sheet。
17. 无外链、宏、重复表名、空主键或公式错误。
18. 日期/百分比/数字/文本格式正确，七Sheet全部渲染目视复核。
19. 过程文件必需项齐全且哈希一致。
20. 唯一问题文档、检查记录和质量报告存在。
21. 独立QA未通过时不得标记`completed/pass`。

交付状态只用`completed、completed_with_gaps、incomplete、blocked`。
