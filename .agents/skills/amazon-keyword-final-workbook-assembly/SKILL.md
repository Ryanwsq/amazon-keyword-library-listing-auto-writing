...
第一板块两Sheet工作簿；第二板块四Sheet工作簿；分类两Sheet、否词库、词频、竞争和趋势过程工作簿；全部manifests/哈希/版本；稳定Keyword_ID；锁定SKU事实卡、`run_type=production|test-validation`和输出目录。production不需要qa_mode或独立QA产物；test-validation再锁定`compact-validation|full-regression`，compact封包锁定`compact-qa-result.json`及按需问题引用，full封包锁定独立QA工作簿、quality manifest、独立预览和问题文档或引用。
