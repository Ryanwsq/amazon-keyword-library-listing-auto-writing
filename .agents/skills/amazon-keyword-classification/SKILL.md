...
3. 按ABA应用互斥F1–F5：1–10k、10–20k、20–50k、50–100k、>100k。ABA缺失时层和LT留空，分类状态写`ABA未提供`。
...
8. 从Sheet3筛选语义明确、复核完成且符合否词收录边界的原始完整关键词，生成五列否词库。流量失败/缺失、歧义和待复核词不自动收录。
9. 验证人口、主键、上游品类关系/通用词库资格逐值不变、F1–F5、动态列清单/顺序、F5唯一主分组、LT<=20、否词来源和禁止字段；渲染所有输出Sheet。
...
主键/人口/版本无法锁定，或Sheet2品类关系/通用词库资格缺失、非法、与上游不一致时停止。ABA缺失不伪造流量层；无法稳定分类时使用动态`其他/未知`并标记复核。否词语义或复核状态不满足时保留在Sheet3，不强行收录。
