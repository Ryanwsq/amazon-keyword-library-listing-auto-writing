...
Amazon联想是必选来源，输入固定使用唯一一级品类核心大词而不是主执行锚点，环境固定为Amazon US未登录、All、10001；首选Codex内置浏览器，当前设备无法稳定识别或操作时允许普通Chrome备用，两个入口都不可用时必须`not_executed`且正常案例未完成。只采集搜索框上方关键词联想区的完整可见关键词建议；底部商品卡、价格卡和商品轮播全部排除。SIF首选MCP并允许同提供商已登录网页备用。卖家精灵关键词挖掘首选其长期副任务内置浏览器中的已登录网页端和完整官方导出，网页不可用时才使用同提供商MCP；查询、周期、过滤、字段和完整性门不变，凭据与会话不得入库。第二板块按动态完整商品对象分流，不按SKU配置删除同品类词；一级品类归属无法确认的歧义词进入Sheet3人工复核，只有一级品类归属已确认但目标细分关系不明时才留在Sheet2并写`关系待复核/待复核`。拼写错误、其他语言和自有品牌不得一律摘除，配置连接词、并列连接和其他语言词序都必须按完整词中心购买对象判断。分类F5按唯一主标签分组且每词只出现一次；`关键词ABA排名缺失、搜索量缺失、没有搜索量`为行级数据状态，不提前停止整批。受影响派生值不得伪造，并进入最终质检用户确认清单；确认未闭合前最终QA为`incomplete`。`品类产品通用词库`只把五个流量块表头显示为`F1 核心大词、F2 二级词、F3 中流量词、F4 中长尾词、F5 长尾词`，内部F1–F5编码、阈值和人口不变。竞争正式输入只有SIF Top3点击份额与Top3转化份额；任一项缺失、冲突未解决或周期不明时不出综合等级，不再使用机会筛选、竞争格局、卖家精灵竞争字段、比较池或样本门。趋势来源优先级为`SellerSprite -> Sorftime`且同一Run只用一个提供商；月度和季度折线图展示实际搜索量，环同比只留表格；月份为空或季度不完整时不补0、不插值、不生成对应图点。P1验收未完成时不得冒充已完成或已验证能力。

## Status integrity

当前唯一清洗基线仍是V2.1；V2.2只标识词频组件。2026-08-20回写、2026-08-21输出合同、2026-08-23核心层级/通用词库资格修正和2026-08-24二类词独立Sheet、输入核心门/卖家精灵入口/任务分离修正均为Post-V2.1增量，不命名为新清洗版本。规则修正前的旧工作簿只能作为旧运行证据；按当前来源入口、分层锚点、单一卖家精灵种子、完整词规则和输出合同完整重跑后才能继续验收。每Run开头由用户确认目标Amazon类目及该类目是否存在多个稳定产品类型细分，用户不指定核心词；主任务结合产品事实卡、直接竞品身份与SIF流量候选独立确认唯一一级品类核心大词。只有用户确认类目存在多个稳定产品类型细分时才可确认最多一个细分核心词，否则必须留空；单独卖点、配置、场景或高流量长词不得建立该层级。有细分核心词时主执行锚点和卖家精灵种子都使用该词，否则使用一级品类核心大词。SIF流量不能把宽泛/相邻词提升为细分核心或强等价。最终交付固定为一个过程文件夹和一个八Sheet最终工作簿；新增`二类词`Sheet机械复制分类完成的Sheet4全人口，不重新判断且允许零行。最终总表仍覆盖三去向全人口并传递`通用词库资格`，否词库只保留语义否词类别且不含否定方式，品类通用词库、词频、竞争和趋势只使用资格为`纳入`的适用人口。广告资格、否定匹配方式和投放动作仍为后置。来源采集已拆为SIF、Amazon联想和卖家精灵三个单一职责Skill，竞争与趋势已拆分，并建立独立质量验证Skill；当前共十二个Skills，全部保持`draft`且能力为`planned`，本次合同同步和P0通过不生成P1证据。
